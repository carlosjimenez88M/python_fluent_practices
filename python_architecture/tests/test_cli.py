...
from news.io.cli import main


class TestCLI(unittest.TestCase):
	"""Test CLI functions."""

	@patch("news.io.cli.NewsService")
	@patch("sys.exit")
	def test_main_search_command(self, mock_exit, mock_service_class):
		...
		with patch("sys.argv", ["news", "search", "test", "--source", "guardian"]):
			main()
	...
	def test_main_ask_command(self, mock_exit, mock_service_class):
		...
		with patch(
			"sys.argv",
			["news", "ask", "test", "question", "--source", "guardian"],
		):
			main()
		mock_service.search_articles.assert_called_once_with("guardian", "test")
	...
	def test_main_no_command(self, mock_exit):
		...
		with patch("sys.argv", ["news"]), self.assertRaises(SystemExit):
			main()
	...
	def test_main_exception_handling(self, mock_exit, mock_service_class):
		...
		with (
			patch("sys.argv", ["news", "search", "test", "--source", "guardian"]),
